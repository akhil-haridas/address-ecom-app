...
exports.getProductc = async (req, res, next) => {
    try {
        ...
        const relate = await Products.find({ category: category.category }).limit(
            4,
        );
    ...
    try {
        if (req.session.user_id) {
            ...
            if (existData.length > 0) {
                const reviewData = await Products.findByIdAndUpdate(
                    { _id: productId },
                    {
                        $push: {
                            reviews: {
                ...
                const addRating = await Products.findByIdAndUpdate(
                    { _id: productId },
                    { $set: { rating: average } },
                );
...
exports.addProduct = async (req, res, next) => {
    try {
        ...
        let imageId = req.files.map((file) => file.path);

        const unique = await Products.findOne({
...
exports.aboutProduct = async (req, res, next) => {
    try {
        const id = req.query.id;
        const productData = await Products.findById({ _id: id }).populate(
            "category",
        );
        const Categories = await Category.find({ __v: 0 });
...
exports.updateProduct = async (req, res, next) => {
    try {
        ...
        const newImageIds = req.files.map((file) => file.path);
